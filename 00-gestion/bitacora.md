...
**3 - Evidencia (corregida).** Extracto exportado manualmente por Tomas desde la consola de FortiSIEM (reporteIncidentes.csv, 3882 filas; columnas Event Receive Time, Event Type, Incident ID, Incident Target, Incident Detail, Event Severity, Case ID). Lectura cuantitativa del dia: 799 alertas de severidad alta/critica -20,6%-, 1111 de severidad media -28,6%-, 1972 de severidad baja -50,8%-. Version anonimizada (ver punto 6) subida a `01-relevamiento/20260916_ReporteIncidentesSIEM_Anonimizado_v1.csv`. El extracto original no incluye el campo Case ID poblado por diseno de ese export puntual; una exportacion separada de FortiSIEM ("Cases", 24/09/2026) muestra que si se generaron casos el 16/09 -20 casos, agrupando 60 Incident ID bajo distintas reglas, todos con fecha de creacion confirmada ese mismo dia-, lo que descarta un lag estructural de dias y corrige la afirmacion previa de "0 alertas con Case ID asignado en la ventana completa de 24 h", que surgia de un artefacto del export usado y no de ausencia real de triage (ver punto 7).
...
**7 - Indicador de cobertura: cruce Incidentes-Casos (dato verificado, no estimado).** Se cruzaron, por Incident ID, las 3882 alertas del 16/09/2026 (reporteIncidentes.csv) contra el export de Cases de FortiSIEM (24/09/2026, 20 casos, 60 Incident ID vinculados). Resultado: 52 de las 3882 alertas -1,34%- estan vinculadas a un caso creado el mismo dia. Desglosado por criticidad: 47 de 799 alertas alta/critica -5,88%-, 5 de 1111 media -0,45%-, 0 de 1972 baja. Archivo con el cruce completo: `01-relevamiento/20260916_CruceIncidentesCasos_CyberShield_v1.csv`.

**8 - Limitacion metodologica explicita.** El indicador solo captura casos CREADOS el 16/09 -verificado: los 20 casos del export tienen fecha de creacion ese dia-. Un incidente asignado a un caso preexistente -abierto en dias anteriores- no queda identificado como atendido con los datos disponibles, porque traer el historial completo de casos del MSSP es inviable para el alcance de la AE2. Por lo tanto, 1,34% es una cota inferior de la atencion real, no la tasa completa de cobertura. Se observo ademas que 8 de los 60 Incident ID vinculados a los 20 casos no aparecen en el extracto del 16/09 por tratarse de incidentes recurrentes cuya ultima ocurrencia real fue el 9 y el 11 de septiembre; esto no afecta el calculo, ya que esos incidentes no pertenecian a la ventana analizada.

**9 - Criterio de mapeo de severidad (confirmado por conteo exacto).** Event Severity 7 a 10 = alta/critica (799 alertas, 20,6%); Event Severity 4 a 6 = media (1111 alertas, 28,6%); Event Severity 1 a 3 = baja (1972 alertas, 50,8%).
